function valueFontSize(value) {
  ...
  if (str.length <= 6) return 'var(--font-size-24)'
  if (str.length <= 10) return 'var(--font-size-20)'
  return 'var(--font-size-16)'
}

export function StatCard({ label, value, unit, sublabel, color, icon, onClick, trend }) {
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, minWidth: 0 }}>
          <span style={{ fontSize: valueFontSize(value), fontWeight: 'var(--font-weight-extra-bold)', color: color || 'rgb(var(--text-color-main))', lineHeight: 1.05, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {value}
          </span>
          {unit ? (
            <span style={{ fontSize: 'var(--font-size-16)', color: 'rgb(var(--text-color-alt))', fontWeight: 'var(--font-weight-semi-bold)' }}>{unit}</span>
          ) : null}
        </div>
        {sublabel ? (
          <span style={{ fontSize: 'var(--font-size-14)', color: 'rgb(var(--text-color-alt))', lineHeight: 1.25 }}>{sublabel}</span>
        ) : null}
      </div>
      {trend != null && (
